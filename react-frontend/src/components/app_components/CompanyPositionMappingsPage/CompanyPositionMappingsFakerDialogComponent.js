...
client.service("positions")
    ...
    const onRun = async () => {
        let fakeData = companyPositionMappingsFakerFactory(props.user,fakerCount,companyIds,positionsIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("companyPositionMappings").create(elem));
...
export default connect(mapState, mapDispatch)(CompanyPositionMappingsFakerDialogComponent);
